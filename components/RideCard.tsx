...
import { ClockIcon, DollarSignIcon, MapPinIcon, CheckCircleIcon, RocketIcon, AlarmClockIcon } from './Icons';
        ...
        <div className="flex justify-between items-center text-sm">
           <div className="flex flex-col gap-1">
             <div className="flex items-center gap-2 text-slate-300">
               ...
               <span>{ride.pickupDate} • {ride.time}</span>
             </div>
             {ride.status === RideStatus.SCHEDULED && ride.scheduledTime && (
               <div className="flex items-center gap-2 text-purple-400 font-bold text-[10px] uppercase tracking-wider">
                 <AlarmClockIcon className="w-3.5 h-3.5" />
                 <span>Disparo: {ride.scheduledTime}</span>
               </div>
             )}
           </div>
