...
import { BellIcon, CheckCircleIcon, CoffeeIcon, TrophyIcon, XCircleIcon, RocketIcon, PowerIcon, UserIcon, MapPinIcon, DollarSignIcon, AlarmClockIcon } from './Icons';
...
const DriverView: React.FC<DriverViewProps> = ({ driver }) => {
  ...
  const scheduledRides = state.rides.filter(r => r.status === RideStatus.SCHEDULED);
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="w-full lg:w-auto">
                    ...
                    >
                        ...
                        <span className="text-lg tracking-wider relative z-10">
                          {driver.isAvailable ? 'ENCERRAR TURNO' : 'DISPONÍVEL PARA ATENDIMENTO'}
                        </span>
                    </button>
                </div>
            </div>
      ...
      <div>
        ...
        {!currentRide && availableRides.some(r => r.offeredToDriverId === driver.id) ? (
      ...
      {scheduledRides.length > 0 && (
        <div className="mt-8">
          <h3 className="text-xl font-bold mb-3 text-purple-400 flex items-center gap-2 uppercase tracking-wider">
              <AlarmClockIcon className="w-6 h-6"/>
              Próximos Chamados (Disparo Automático)
          </h3>
              ...
              <div key={ride.id} className="relative">
                <RideCard ride={ride}>
                  <div className="mt-4 p-3 bg-purple-500/10 border border-purple-500/20 rounded-lg">
                    <p className="text-[10px] text-purple-300 uppercase font-black tracking-widest text-center">
                      O sistema enviará este chamado para a fila às <span className="text-white text-xs">{ride.scheduledTime}</span>
                    </p>
                  </div>
                </RideCard>
              </div>
              ...
              <div key={ride.id} className="flex justify-between items-center p-4 bg-slate-700/20 rounded-xl border border-slate-600/30 hover:bg-slate-700/40 transition-colors">
                <div className="flex items-start gap-3 flex-grow min-w-0">
                  <div className="flex flex-col gap-1 shrink-0 mt-1">
                    ...
                    <MapPinIcon className="w-3 h-3 text-rose-400" />
                  </div>
                  <div className="flex flex-col min-w-0">
                    <span className="text-xs font-medium text-slate-400 truncate uppercase tracking-tighter">De: {ride.pickup}</span>
                    <span className="text-sm font-bold text-slate-100 truncate">Para: {ride.destination}</span>
